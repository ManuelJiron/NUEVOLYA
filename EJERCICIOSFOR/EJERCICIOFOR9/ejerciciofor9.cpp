...
int main(int argc, char const *argv[])
{
    ...
    vector<vector<int>> matrizResultado(filas1, vector<int>(columnas2, 0)); // Calcular la matriz resultante
    for (int i = 0; i < filas1; ++i)
        for (int j = 0; j < columnas2; ++j)
            for (int k = 0; k < columnas1; ++k)
                matrizResultado[i][j] += matriz1[i][k] * matriz2[k][j];

    cout << "La matriz resultante de la multiplicación es:" << endl; // Mostrar la matriz resultante
